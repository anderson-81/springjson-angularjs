<%@taglib prefix="t" tagdir="/WEB-INF/tags" %>
<t:_layout>
    <jsp:attribute name="body_area">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h1>SpringJson</h1>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div class="jumbotron">
                        <p>CRUD made in Java, AngularJS, Spring Framework, Hibernate with PostgreSQL database.</p>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <img src="../../../resources/images/logos/postgresql.png" alt="" class="postgresql pull-right"/>
                        <img src="../../../resources/images/logos/hibernate.png" alt="" class="hibernate  pull-right"/>
                        <img src="../../../resources/images/logos/spring.png" alt="" class="spring  pull-right"/>
                        <img src="../../../resources/images/logos/angularjs.png" alt="" class="angularjs pull-right"/>
                        <img src="../../../resources/images/logos/java.png" alt="" class="java  pull-right"/>
                    </div>
                </div>
            </div>
        </div>
        <link href="../../../resources/styles/home.css" rel="stylesheet" type="text/css"/>
    </jsp:attribute>
</t:_layout>